...
import { Save, Info, TrendingUp, RotateCcw } from "lucide-react";
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[var(--anna-border)] bg-[var(--anna-bg)] sticky top-0">
                <th className="text-left px-5 py-2.5 text-[10px] font-semibold uppercase tracking-wider text-[var(--anna-muted)]">Category</th>
                <th className="text-right px-5 py-2.5 text-[10px] font-semibold uppercase tracking-wider text-[var(--anna-muted)]">Default</th>
                <th className="text-right px-5 py-2.5 text-[10px] font-semibold uppercase tracking-wider text-[var(--anna-muted)]">Current Price</th>
                <th className="text-center px-5 py-2.5 text-[10px] font-semibold uppercase tracking-wider text-[var(--anna-muted)]">Status</th>
                <th className="text-right px-5 py-2.5 text-[10px] font-semibold uppercase tracking-wider text-[var(--anna-muted)]">Vendor Payout</th>
              </tr>
            </thead>
            <tbody>
              {categoryPricing.map((p) => {
                ...
                return (
                  <tr key={p.category as string} className={cn("border-b border-[var(--anna-border)] last:border-0 transition-colors", isActive ? "hover:bg-[var(--anna-sage-light)]/30" : "opacity-50")}>
                    ...
                    <td className="px-5 py-2.5 text-right font-data text-xs text-[var(--anna-muted)]">{formatSgd(p.defaultPriceCents as number)}</td>
                    <td className="px-5 py-2.5 text-right">
                      {isAdmin && isActive ? (
                        <div className="flex items-center justify-end gap-1">
                          <span className="text-xs text-[var(--anna-muted)]">SGD $</span>
                          <Input type="number" min={0} step={100} value={Math.round(currentPrice / 100)} onChange={(e) => onPriceChange(p.category as string, (parseInt(e.target.value) || 0) * 100)} className={cn("w-20 h-7 text-right text-xs font-data rounded-lg border-[var(--anna-border)]", changed && "border-[var(--anna-sage)] bg-[var(--anna-sage-light)]/30")} />
                        </div>
                      ) : (
                        <span className={cn("font-data text-xs", isCustom ? "text-[var(--anna-sage-dark)] font-semibold" : "text-[var(--anna-slate)]")}>{formatSgd(p.activePriceCents as number)}</span>
                      )}
                    </td>
                    <td className="px-5 py-2.5 text-center">
                      {isCustom ? <Badge variant="secondary" className="text-[8px] bg-[var(--anna-sage-light)] text-[var(--anna-sage-dark)]">Custom</Badge> : <Badge variant="secondary" className="text-[8px] bg-[var(--anna-bg)] text-[var(--anna-muted)]">Default</Badge>}
                    </td>
                    <td className="px-5 py-2.5 text-right font-data text-xs text-emerald-700">{formatSgd(vendorPayout)}</td>
                  </tr>
                );
              })}
            </tbody>
