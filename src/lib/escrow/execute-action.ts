...
// ─────────────────────────────────────────────────────────────
// Phase 2 · THE ONE MONEY EXECUTION PATH for escrow/dispute
// actions (user spec §6).
//
// This is the verbatim body of the former PATCH handler of
// /api/ops/escrow/[id], extracted so that BOTH entry points —
// the manual ops dialog (PATCH route) and the AI-assisted
// decision route — execute through the SAME implementation with
// the SAME guards:
//
//   • refundConfirmed maker-checker 409 gate (intact, enforced
//     here for every caller — no AI backdoor)
//   • resolution-note requirement on refund-class actions
//   • state guards, claim-first concurrency, idempotency,
//     commission/payout recalculation, notifications, events,
//     anomaly resolution — all unchanged
//
// The AI layer PREPARES a payload and hands control to this
// function; it never moves money itself. There is no second
// refund service.
//
// Phase 2 addition (pure bookkeeping, no money semantics): after
// a successful action, any still-PENDING_REVIEW AiCaseBrief for
// the task is expired — events moved on before a human decided.
// ─────────────────────────────────────────────────────────────
...
export async function executeEscrowAction(
  ...
  try {
    const { action, resolution, refundAmountCents, idempotencyKey,
            voucherAmountCents, voucherRefundAmountCents, voucherExpiryDays, refundConfirmed } = input;
    ...
    if (action === "release") {
      ...
      // ── Phase 2: expire any still-pending AI case briefs for this task
      // (events moved on before a human decided). Pure bookkeeping —
      // never blocks or fails the money action.
      try {
        const { expirePendingBriefsForTask } = await import("@/lib/ai-dispute/brief-service");
        await expirePendingBriefsForTask(
          task.id,
          `Escrow action '${action}' executed by ${session.name}`
        );
      } catch (e) {
        console.warn("[executeEscrowAction] brief expiry hook failed (non-fatal):", e);
      }
    ...
    console.error("[executeEscrowAction]", error);
    return NextResponse.json(
